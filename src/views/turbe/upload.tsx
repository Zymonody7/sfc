...
import { getPreList } from "@/api/modules/turbe";
...
const TurbeUpload = () => {
	...
	const handleSubmit = async () => {
		const res = await getPreList(11);
		const x = res.map(item => item.DATATIME);
		const y = res.map(item => item.YD15);
		console.log(x, y);
		const newOption = cloneDeep(lineOption);
